...
class CanvasService
{
    ...
    public static function renderToImage(array $canvasData, int $conversationId): array
    {
        ...
        try {
            ...
            if (!$image) {
                return ['success' => false, 'error' => 'Falha ao criar imagem. Extensão GD pode não estar disponível.'];
            }
            ...
            foreach ($objects as $obj) {
                ...
                switch ($type) {
                    case 'image': self::renderImage($image, $obj); break;
                    case 'text': case 'i-text': case 'textbox': self::renderText($image, $obj); break;
                    case 'rect': self::renderRect($image, $obj); break;
                    case 'circle': self::renderCircle($image, $obj); break;
                    case 'line': self::renderLine($image, $obj); break;
                }
            }

            $filename = 'canvas_' . \uniqid() . '_' . \time() . '.png';
    ...
    private static function renderImage($canvas, array $obj): void
    {
        try {
            ...
            @\imagecopyresampled($canvas, $sourceImage, $left, $top, 0, 0, $destWidth, $destHeight, $srcWidth, $srcHeight);
    ...
    private static function getFontPath(string $fontFamily): ?string
    {
        ...
        if (\file_exists($defaultFont)) return $defaultFont;

        return null;
    }

    private static function createThumbnail($source, int $maxWidth)
    {
        try {
    ...
    public static function renderWithNode(array $canvasData, int $conversationId): array
    {
        try {
            ...
            if (!\file_exists($nodeScript)) {
                @\unlink($tempJson);
                return self::renderToImage($canvasData, $conversationId);
            }
