...
function getChannelIconSvg($channel, $size = 16) {
    $icons = [
        ...
        'instagram_comment' => '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size . '" viewBox="0 0 24 24" fill="url(#instagram-gradient-comment-php)" style="vertical-align: middle;"><defs><linearGradient id="instagram-gradient-comment-php" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style="stop-color:#833AB4;stop-opacity:1" /><stop offset="50%" style="stop-color:#FD1D1D;stop-opacity:1" /><stop offset="100%" style="stop-color:#FCAF45;stop-opacity:1" /></linearGradient></defs><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z"/></svg>',
...
function getChannelInfo(channel) {
    const channels = {
        ...
        'instagram_comment': {
            name: 'Comentário',
            icon: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="url(#instagram-gradient-comment)" style="vertical-align: middle;"><defs><linearGradient id="instagram-gradient-comment" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style="stop-color:#833AB4;stop-opacity:1" /><stop offset="50%" style="stop-color:#FD1D1D;stop-opacity:1" /><stop offset="100%" style="stop-color:#FCAF45;stop-opacity:1" /></linearGradient></defs><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z"/></svg>',
            emoji: '💬'
        },
        'facebook': {
            name: 'Facebook',
                    ...
                    <div class="mb-5">
                        ...
                        <div class="border rounded p-3" style="max-height: 300px; overflow-y: auto; background: var(--bs-gray-100);">
                            ...
                            $availableChannels = [
                                ...
                                'instagram_comment' => ['icon' => getChannelIconSvg('instagram', 18), 'name' => 'Comentário Instagram'],
...
function refreshConversationList(params = null) {
    ...
    .then(data => {
        ...
        conversations.forEach(conv => {
            // 🔥 PROTEÇÃO: Pular conversas sem dados completos (evita "NN" fantasma)
            if (!conv.contact_name || conv.contact_name === 'NN' || !conv.contact_id) {
                console.warn('⚠️ Conversa incompleta no refresh, pulando:', conv.id);
                return; // Pula esta conversa
            }
            ...
            const nameRaw = conv.contact_name;
            const maxName = 22;
...
function addConversationToList(conv) {
    ...
    // 🔥 PROTEÇÃO: Não adicionar conversas sem dados completos (evita "NN" fantasma)
    if (!conv.contact_name || conv.contact_name === 'NN' || !conv.contact_id) {
        console.warn('⚠️ Conversa incompleta recebida (sem contact_name), aguardando dados completos...', conv);
        // Agendar retry em 500ms para buscar dados completos
        setTimeout(() => {
            // Buscar conversa atualizada via AJAX
            fetch(`/conversations/${conv.id}`, {
                ...
                .then(r => r.json())
                .then(data => {
                    if (data.success && data.conversation && data.conversation.contact_name) {
                        console.log('✅ Dados completos recebidos, adicionando conversa:', data.conversation);
                        addConversationToList(data.conversation);
                    }
                })
                .catch(err => console.error('Erro ao buscar conversa completa:', err));
        }, 500);
        return;
    }
